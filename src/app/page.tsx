...
import {
  ...
  BarChart3, ChevronLeft, ChevronRight, Flame,
  Keyboard,
} from 'lucide-react'
import { cn } from '@/lib/utils'

type AppView = 'landing' | 'auth' | 'app'
type ActivePanel = 'timer' | 'music' | 'tasks' | 'stats'
...
export default function Home() {
  ...
  // Derive app view from auth state
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === '1') setActivePanel('timer')
      if (e.key === '2') setActivePanel('music')
      if (e.key === '3') setActivePanel('tasks')
      if (e.key === '4') setActivePanel('stats')
    }
    window.addEventListener('keydown', handler)
    return () => window.removeEventListener('keydown', handler)
  }, [appView])

  // Format timer for sidebar mini display
  ...
  const navItems = [
    { id: 'timer' as const, icon: Timer, label: 'Timer', shortcut: '1' },
    { id: 'music' as const, icon: Music, label: 'Focus Sounds', shortcut: '2' },
    { id: 'tasks' as const, icon: ListTodo, label: 'Tasks', shortcut: '3' },
    { id: 'stats' as const, icon: BarChart3, label: 'Analytics', shortcut: '4' },
  ]
        ...
        >
          ...
          <div className="relative z-10 flex h-screen">
            ...
            >
              ...
              {/* ─── Mini Timer Display ─── */}
              <div className={cn(
                'px-3 py-4 border-b border-border/15',
                  ...
                  <div className={cn(
                    ...
                    <div className="relative flex-shrink-0">
                      <svg width={sidebarCollapsed ? 36 : 40} height={sidebarCollapsed ? 36 : 40} className="transform -rotate-90">
                        <circle
                          cx={sidebarCollapsed ? 18 : 20}
                          cy={sidebarCollapsed ? 18 : 20}
                          r={sidebarCollapsed ? 14 : 16}
                          fill="none"
                          stroke="currentColor"
                          strokeWidth="2.5"
                          ...
                          <div className="flex items-center gap-1.5 mt-1">
                            ...
                            {sessionsCompleted > 0 && (
                              <>
                                ...
                                <span className="text-[10px] text-amber-500/70">{sessionsCompleted} done</span>
                              </>
                            )}
                          </div>
                        </motion.div>
                      ...
                      <AnimatePresence>
                        {!sidebarCollapsed && (
                          ...
                          >
                            ...
                            <span className={cn(
                              'text-[10px] font-mono opacity-0 group-hover:opacity-40 transition-opacity',
              ...
              {/* ─── Bottom Section ─── */}
              <div className={cn(
                'border-t border-border/20 px-2 py-3 space-y-1',
              )}>
                {/* Theme toggle */}
                <button
                      ...
                      >
                        ...
                        <div className="p-1">
                          <button
                            ...
                            className="w-full flex items-center gap-2 px-2.5 py-2 rounded-lg text-xs text-destructive hover:bg-destructive/10 transition-colors"
                          >
            ...
            <main className="flex-1 flex flex-col h-full overflow-hidden">
              {/* Top bar with breadcrumb-style header */}
              <header className="flex items-center justify-between px-6 h-14 border-b border-border/20 flex-shrink-0">
                <div className="flex items-center gap-3">
                  <h2 className="text-sm font-semibold">
                    {activePanel === 'timer' && 'Focus Timer'}
                    {activePanel === 'music' && 'Ambient Sounds'}
                    {activePanel === 'tasks' && 'Task Manager'}
                    {activePanel === 'stats' && 'Analytics'}
                  </h2>
              ...
              <nav className="lg:hidden flex items-center justify-around py-2 px-4 border-t border-border/30 bg-background/80 backdrop-blur-lg flex-shrink-0">
                {navItems.map((item) => {
                  const isActive = activePanel === item.id
                  const Icon = item.icon
                  return (
                    ...
                    >
                      ...
                      <span className="text-[10px] font-medium">{item.label === 'Focus Sounds' ? 'Sounds' : item.label}</span>
                      {isActive && (
                        <motion.div
